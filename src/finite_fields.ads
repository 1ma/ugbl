package Finite_Fields is
   ...
   function FF_Add (A, B : Integer; O : Order) return Integer
      with Post => FF_Add'Result >= 0 and FF_Add'Result < Integer (O);

   function FF_Sub (A, B : Integer; O : Order) return Integer
      with Post => FF_Sub'Result >= 0 and FF_Sub'Result < Integer (O);

   function FF_Mul (A, B : Integer; O : Order) return Integer
